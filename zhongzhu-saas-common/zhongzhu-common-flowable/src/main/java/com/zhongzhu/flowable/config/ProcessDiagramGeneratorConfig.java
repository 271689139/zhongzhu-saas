package com.zhongzhu.flowable.config;

import com.zhongzhu.core.i18n.ObjectUtil;
import com.zhongzhu.core.utils.BigDecimalUtil;
...
public class ProcessDiagramGeneratorConfig extends DefaultProcessDiagramGenerator {
        ...
        while (true) {
            ...
            while (true) {
                ...
                do {
                    ...
                    } while (ObjectUtil.isNotNull(graphicInfo) && ObjectUtil.isNotNull(graphicInfo.getExpanded()) && !graphicInfo.getExpanded());
    ...
    private static ProcessDiagramCanvasConfig initProcessDiagramCanvas(double minX, double maxX, double minY, double maxY, BpmnModel bpmnModel, String imageType, String activityFontName, String labelFontName, String annotationFontName, ClassLoader customClassLoader) {
        ...
        return new ProcessDiagramCanvasConfig((int) maxX + 10, (int) maxY + 10, (int) minX, (int) minY, imageType, activityFontName, labelFontName, annotationFontName, customClassLoader);
    }
    ...
    protected void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode, List<String> highLightedActivities, List<String> highLightedFlows, double scaleFactor, Boolean drawSequenceFlowNameWithNoLabelDi) {
        ...
        for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
            ...
            boolean drawConditionalIndicator = ObjectUtil.isNotNull(sequenceFlow.getConditionExpression()) && !(flowNode instanceof Gateway);
        ...
        if (flowNode instanceof FlowElementsContainer) {
            for (FlowElement nestedFlowElement : ((FlowElementsContainer) flowNode).getFlowElements()) {
                if (nestedFlowElement instanceof FlowNode && !isPartOfCollapsedSubProcess(nestedFlowElement, bpmnModel)) {
                    drawActivity(processDiagramCanvas, bpmnModel, (FlowNode) nestedFlowElement, highLightedActivities, highLightedFlows, scaleFactor, drawSequenceFlowNameWithNoLabelDi);
                }
            }
        }
    }
            ...
            if (highLightedActivities.contains(flowNode.getId())) {
                ...
                if (highLightedActivities.getLast().equals(flowNode.getId()) && !"endenv".equals(flowNode.getId())) {
                    ...
                    if ((flowNode.getId().contains(eventPrefix))) {
                        drawHighLightEnd((ProcessDiagramCanvasConfig) processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()));
                    } else {
                        drawHighLightNow((ProcessDiagramCanvasConfig) processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()));
                    }
                } else {
